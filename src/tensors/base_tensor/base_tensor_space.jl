...
# Examples
```jldoctest
julia> DiffusionGeometryJL._is_orthonormal(function_space(dg))
true

julia> DiffusionGeometryJL._is_orthonormal(vector_field_space(dg))
false
```
"""
...
```jldoctest
julia> vals, vecs = DiffusionGeometryJL._gram_spectrum(vector_field_space(dg));
