...
```jldoctest
julia> c = DiffusionGeometryJL._from_pointwise_basis(reshape(cos.(θ), 60, 1), function_space(dg));
...
```jldoctest
julia> values = DiffusionGeometryJL._to_pointwise_basis(f.coeffs, function_space(dg); basis_count=8);
